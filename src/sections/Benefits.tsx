...
const Benefits = () => {
  ...
  return (
    <section
      id="benefits"
      ref={sectionRef}
      className="scroll-reveal relative overflow-hidden bg-slate-950 py-24"
    >
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr,0.9fr] lg:items-center">
          <div className="grid gap-6">
            {benefits.map((benefit) => (
              <article
                key={benefit.title}
                className="group rounded-3xl border border-white/10 bg-white/5 p-8 transition hover:border-brand-400/40"
              >
                <h3 className="text-lg font-semibold text-white">{benefit.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-slate-400">{benefit.description}</p>
              </article>
            ))}
          </div>
          <div className="relative mx-auto w-full max-w-md">
            <div className="absolute -inset-6 rounded-[40px] bg-brand-500/10 blur-3xl" />
            <div className="relative overflow-hidden rounded-[32px] border border-white/10 bg-gradient-to-br from-slate-900 to-slate-950 p-8 shadow-section">
              <div className="space-y-6">
                ...
                <div className="space-y-4 rounded-3xl bg-black/40 p-6 ring-1 ring-white/10">
                  {[
                    ...
                    <div key={metric.label} className="space-y-2">
                      <div className="flex items-center justify-between text-xs text-slate-300">
                        ...
                        <span className="font-semibold text-brand-200">{metric.value}</span>
                      </div>
