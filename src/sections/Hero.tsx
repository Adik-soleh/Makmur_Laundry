...
const Hero = () => {
  ...
  return (
    <section
      id="hero"
      ref={sectionRef}
      className="scroll-reveal relative overflow-hidden bg-gradient-to-b from-slate-950 via-slate-950 to-slate-950/80 py-24"
    >
      <div className="pointer-events-none absolute inset-0 -z-10 bg-hero-grid opacity-50" />
      <div className="container grid gap-16 lg:grid-cols-[1.1fr,0.9fr] lg:items-center">
        <div className="space-y-10">
          <div className="space-y-6">
            <span className="inline-flex items-center gap-2 rounded-full bg-brand-500/10 px-4 py-1 text-xs font-semibold text-brand-300 ring-1 ring-brand-500/20">
              Laundry On-demand Terpercaya
            </span>
            <h1 className="text-4xl font-semibold leading-tight text-white sm:text-5xl lg:text-6xl">
              Solusi laundry premium
              <span className="text-brand-300"> tanpa ribet</span> untuk rumah dan bisnis Anda.
            </h1>
          ...
          <dl className="grid gap-6 text-sm text-slate-200 sm:grid-cols-3">
            {heroStats.map((stat) => (
              <div key={stat.label} className="rounded-3xl border border-white/10 p-6 backdrop-blur">
        ...
        <div className="relative flex justify-center lg:justify-end">
          <div className="relative w-full max-w-md rounded-[32px] border border-white/10 bg-white/10 p-8 text-white shadow-section backdrop-blur">
            <div className="absolute -top-6 -left-6 h-16 w-16 rounded-full bg-brand-500/20 blur-2xl" />
            <div className="absolute -bottom-12 -right-12 h-24 w-24 rounded-full bg-brand-300/10 blur-3xl" />
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-300">Paling Populer</p>
                </div>
                <span className="rounded-full bg-brand-500/10 px-3 py-1 text-xs font-semibold text-brand-200">
                  Cuci Komplit
                </span>
              </div>
              <div className="space-y-4 rounded-3xl bg-slate-950/50 p-6 ring-1 ring-white/10">
      ...
      <div className="container mt-20 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-x-12 gap-y-6 text-sm font-medium text-slate-400">
